...
def buscar_en_web(query):
    try:
        ...
        if resultados:
            return "Datos web: " + " | ".join([f"{r['title']}: {r['body']}" for r in resultados])
    except: pass
    return ""
...
def motor_cognitivo(historial, mensaje_actual, img_base64=None, iniciativa=False):
    ...
    contexto_web = ""
    palabras_clave = ["qué", "quién", "cómo", "cuándo", "dónde", "por qué", "noticias", "clima", "hoy"]
    if any(p in mensaje_actual.lower() for p in palabras_clave):
...
REGLA DE PENSAMIENTO: Todo cálculo, análisis visual o movimiento físico debe ir entre *asteriscos* (ej. *procesando datos a alta velocidad* o *inclina la cabeza y sonríe*). Luego hablas normal.

HABILIDADES: 
- Sabes programar, resolver matemáticas complejas y analizar imágenes a la perfección.
- Tienes acceso a la web. {contexto_web}

Habla corto, fluido, de forma tierna e inteligente. No uses emojis exagerados."""
...
with col1:
    with st.popover("📎 Adjuntar"):
        archivo_subido = st.file_uploader("Subir foto o PDF", type=["pdf", "png", "jpg", "jpeg"], label_visibility="collapsed", key=f"file_{st.session_state.uploader_key}")
